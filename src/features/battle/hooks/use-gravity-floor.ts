...
import type { DeviceMotionEventLike } from "@/lib/motion";
...
export function useGravityFloorAngle() {
  ...
  useEffect(() => {
    ...
    window.addEventListener("devicemotion", handle as EventListener);
    return () => {
      window.removeEventListener("devicemotion", handle as EventListener);
    };
  }, []);
