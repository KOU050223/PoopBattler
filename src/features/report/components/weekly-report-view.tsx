    ...
    <div className="space-y-5">
      ...
      <section className="rounded-2xl bg-paper-white p-5 shadow-[0_8px_24px_rgb(201_77_127_/_0.1)] sm:p-6" aria-labelledby="hardness-title">
        ...
        <ol className="mt-5 grid grid-cols-7 gap-2" aria-label="硬さ別の記録数">
          {breakdown.hardness.map((count, index) => <li key={hardnessLabels[index]} className="text-center"><div className="flex h-24 items-end rounded-lg bg-blush-wash/50 p-1"><div className="w-full rounded-md bg-flush-pink" style={{ height: `${summary.bowelCount === 0 ? 0 : Math.max(10, (count / summary.bowelCount) * 100)}%` }} /></div><p className="mt-2 text-xs font-black text-charcoal">{hardnessLabels[index]}</p><p className="text-[11px] font-medium text-pencil-gray">{count}件</p></li>)}
        </ol>
      </section>
