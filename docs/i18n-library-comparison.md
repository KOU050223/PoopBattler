...
| 日本語文言を含むファイル | 40（テスト除く） | `grep -rlP '[\p{Hiragana}\p{Katakana}]'` |
| 実文言のおおよその数 | **約 130 行**（テスト・コメント除外後） | 下記注記 |
...
文言数の数え方：`grep -rnP '[\p{Hiragana}\p{Katakana}]' src` の生の結果は 515 行だが、
これはテストファイルと日本語コメントを含む上限値である。`.test.` とコメント行を除外すると
約 130 行になり、これが実際に翻訳対象となる UI 文言の目安。
...
なお「Cookie 方式は静的レンダリングを捨てることになる」という一般的な指摘は、
**このリポジトリには当てはまらない**。`src/proxy.ts` が全リクエストで Supabase セッションを
更新しており、既に動的レンダリングだからである。つまり Cookie 方式の主な欠点が
このアプリでは実質的なコストにならない。
...
結果として「10言語」はコード修正量ではなく **翻訳 JSON を追加する作業**に収束する。
第 5 節の手順 3 まで終われば、以降の言語追加でコードを触る必要はほぼない。
...
5. **2言語目の追加**：`messages/en.json` と言語切り替え UI。ここで初めて ICU の複数形が要る。
   1言語目と2言語目の間には配線の差があるが、**2言語目と10言語目の間には差が無い**。
   以降は JSON を追加するだけで増やせる。
