...
| 認証・DB | Supabase | 匿名ログイン、Postgres |
...
8. 食事画像はIndexedDBに置き、`meal_logs.image_path` には画像本体ではなくローカル画像IDだけを保存する。画像はサーバーへ送信せず、同じ端末・ブラウザプロファイル内でのみ参照できる。
...
1. Next.jsの初期化、Tailwind、Supabase接続、Anonymous Sign-Ins、RLS、`src/proxy.ts`。
