...
3. 端末の揺れがしきい値を超えたら敵にダメージを与える。
4. バトル中に「あげる」を選ぶとカメラを起動し、AR風画面で直近の食事写真を敵に与える。
   与えた食事が敵の属性を確定させ、バトルを有利にする。食事ログが空ならこの導線は出さない。
5. 撃破後に便の硬さ・量・色・出しやすさを選択する。
6. Server Actionが仲間化を抽選・保存し、カメラ画面上で「便器から這い出てくる」演出を出す。
...
- カメラはバトル中も使うが、常時起動はしない。「あげる」を選んだ間だけ `getUserMedia()` を開き、
  操作が終わったらストリームを停止してバトル画面へ戻す。カメラ表示中は揺れによる攻撃判定を止める。
